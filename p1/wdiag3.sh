...
KTHREAD_NAME='napi/enp195s0np0-8263'
...
case "${1:-}" in
  ...
  cell)
    ...
    reset_pins
    echo "pin target: $KTHREAD_NAME pid=$(eval "$KTHREAD_CMD")" > "$O/pin-target.txt"
    ...
    for pair in 10:32704 11:32726 12:32706 13:32724 14:32725; do
      o=${pair%%:*}; s=${pair##*:}
      ssh -n -o ControlPath=/tmp/p1mux-%r@%h -o ControlPersist=600 -o StrictHostKeyChecking=no -o ConnectTimeout=8 davidlin@10.10.1.$o "sudo bash -c 'nohup /root/k2/k5blast --dip 10.10.1.1 --sip $o --sport $s --dport 7777 --rate 158000 --secs $((DUR + 3)) --plen 64 --core 4 > /tmp/wdiag-snd-$o.txt 2>&1 </dev/null &'"
      sleep 0.4
    done
    sleep "$DUR"
    for o in 10 11 12 13 14; do ssh -n -o StrictHostKeyChecking=no -o ConnectTimeout=8 davidlin@10.10.1.$o 'sudo pkill -xc k5blast; true' 2>/dev/null || true; done
    ...
    PP=$(awk '/ch7_poll/{print $2}' "$O/counters-pre.txt")
    PQ=$(awk '/ch7_poll/{print $2}' "$O/counters-post.txt")
    WP=$(awk '/rx_packets_phy/{print $2}' "$O/counters-pre.txt")
    WQ=$(awk '/rx_packets_phy/{print $2}' "$O/counters-post.txt")
    echo "ch7_poll delta = $((PQ - PP)); wire (rx_packets_phy) delta = $((WQ - WP))"
    if [ $((WQ - WP)) -lt 100000 ]; then
      echo "GATE-FAIL: wire saw almost no traffic ($((WQ - WP)) pkts) -- sender failure, cell INVALID"
    elif [ $((PQ - PP)) -lt 100000 ]; then
      echo "WEDGE-CLASS: wire full ($((WQ - WP)) pkts) but the driver processed almost nothing -- onset near t=0, VALID data"
    fi
