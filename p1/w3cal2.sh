...
run_arm() {
  ...
  echo "== loadonly =="
  ssh -n -o StrictHostKeyChecking=no -o ConnectTimeout=8 davidlin@$MA "sudo /root/k2/mutilate -s 10.10.1.1 --loadonly -K 30 -V 32 -r 100000 -T 4" > $O/loadonly-$AR.txt 2>&1
  echo "== 20k QPS cost window (60 s measure) =="
  ssh -n -o StrictHostKeyChecking=no -o ConnectTimeout=8 davidlin@$MA "sudo bash -c 'nohup /root/k2/mutilate -s 10.10.1.1 --noload -T 4 -c 16 -d 64 -K 30 -V 32 -u 0.1 -r 100000 -i exponential -q 20000 -w 5 -t 65 -C 2 -Q 500 -D 4 -a 10.10.1.10 -a 10.10.1.11 -a 10.10.1.12 -a 10.10.1.13 -a 10.10.1.14 --save /tmp/w3-samples-$AR.txt > /tmp/w3-load-$AR.txt 2>&1 </dev/null &'"
  sleep 6
  ...
  ssh -n -o StrictHostKeyChecking=no -o ConnectTimeout=8 davidlin@$MA "sudo /root/k2/mutilate -s 10.10.1.1 --noload -T 2 -c 4 -d 4 -K 30 -V 32 -u 0.1 -r 100000 -i exponential -q 200 -w 5 -t 35 -C 2 -Q 200 -D 4 -a 10.10.1.10" > $O/idle-$AR.txt 2>&1
  echo "== landing gate: queue deltas under a 5 s trickle =="
  C0=$(ethtool -S $IFACE | grep -E 'rx7_packets:|rx0_packets:|rx1_packets:')
  ssh -n -o StrictHostKeyChecking=no davidlin@$MA "sudo /root/k2/mutilate -s 10.10.1.1 --noload -T 1 -c 2 -d 2 -K 30 -V 32 -u 0.1 -r 100000 -q 5000 -w 1 -t 6 -C 2 -Q 500 -a 10.10.1.10" > /dev/null 2>&1
