...
One cell = 5 minutes: a sampled configuration, a sampled load
schedule, one sampled perturbation during load, then the oracle pass
over the 1 Hz counters. Rings and channels are set between cells only.
Hits go to results/p2-RXFUZZ/<date>/<cell>/ and hits.csv.
"""
import argparse, datetime, json, os, random, re, subprocess, time

IFACE = "enp195s0np0"
IRQ = "312"
REF = 525000                      # the Fig. 1 contract knee (pps)
...
DIMENSIONS = {
    ...
    "adaptive_rx": ["on", "off"],
    "ring": ["default", "8192"],                      # set between cells only
    "striding_rq": ["on", "off"],                     # set between cells only
    "gro": ["on", "off"],
    "busy_read": ["0", "50"],
...
SCHEDULES = {
    ...
    3: ("five cycles of 20 s 1.5x / 20 s 0.3x",
        [(1.5, 20), (0.3, 20)] * 5),
...
def napi_threads():
    ...
    for p in os.listdir("/proc"):
        ...
        try:
            ...
            continue
        if c.startswith(f"napi/{IFACE}-"):
            out.append(int(p))
    return out

def cpu_topology():
    """(irq_cpu, smt_sibling, same_l3, other_l3) derived, not guessed"""
    irq_cpu = 8
...
def apply_config(cfg):
    """config at cell start; ring/striding are between-cells only and are
    applied by the caller before the cell (never mid-cell)"""
    ...
    if cfg["napi_placement"] != "unpin":
        sh(f"echo {want} > /proc/irq/{IRQ}/smp_affinity_list")
    else:
...
def perturb(rng):
    """one perturbation during load (never channels or rings)"""
...
def parse_counters(path):
    ...
    for ln in open(path, errors="replace"):
        m = re.match(r"@ (\S+)", ln)
        if m: t = float(m.group(1)); out.append([t, None, None]); continue
        if t is None: continue
        m = re.search(r"rx7_packets: (\d+)", ln)
        if m: out[-1][1] = int(m.group(1))
        m = re.search(r"rx_out_of_buffer: (\d+)", ln)
        if m: out[-1][2] = int(m.group(2) if False else m.group(1))
    return [r for r in out if r[1] is not None]

def oracles(counters, offered_by_time, drop_time):
    """the spec's four oracles over the 1 Hz counters"""
    hits = set()
    flat = 0
    for i in range(2, len(counters)):
        t, rx7, oob = counters[i]
        d_rx = rx7 - counters[i-1][1]
        d_oob = oob - counters[i-1][2]
        if d_rx == 0 and d_oob > 0: flat += 1
        else: flat = 0
        if flat >= 6:
            hits.add("STALL")
...
def probe():
    """the task 1 probe: 30 s at 10k, advance >= 270000 means alive"""
    sh("ssh -n -o StrictHostKeyChecking=no -o ConnectTimeout=8 davidlin@10.10.1.10 "
       ...
       "--dport 7777 --rate 10000 --secs 30 --plen 64 --core 4 > /tmp/rxfuzz-probe.txt 2>&1 </dev/null &'\"")
...
def run_cell(date, cell_id, cfg, sched_id, rng, smoke=False):
    ...
    sh("/root/k2/k2_rx --port 7777 --core 8 --secs 400 --skip 0 > /dev/null 2>&1 &")
    time.sleep(1)
    ...
    for frac, secs in segs:
        ...
        while elapsed < secs:
            ...
            if not smoke and (done + elapsed) / max(1, sum(s for _, s in segs)) >= pert_at:
                what = perturb(rng)
        ...
        time.sleep(0.2)
    adv = probe()
    if adv < 270000: hits = {"PERMANENT"}
    else: hits = set()
    cw.terminate(); time.sleep(1)
    counters = parse_counters(f"{d}/counters.log")
    hits |= oracles(counters, offered, drop_time)
    sh(f"dmesg -T | tail -40 > {d}/dmesg.txt")
    open(f"{d}/verdict.txt", "w").write(
        f"probe_adv={adv}\nhits={sorted(hits)}\nconfig={json.dumps(cfg)}\nschedule={sched_id}\n")
    if hits:
        hitstr = "|".join(sorted(hits))
        sh(f"echo '{date},{cell_id},{hitstr},{sched_id},{json.dumps(cfg)},probe={adv}' >> {ROOT}/hits.csv")
        for cls in hits:
            mark = f"{ROOT}/.trace-done-{cls}"
            if not os.path.exists(mark):
                open(mark, "w").write(cell_id)   # the 10 s trace hook runs here in v1
...
def main():
    ...
    if a.mode == "run":
        ...
        for i in range(1, a.cells + 1):
            ...
            hits, adv = run_cell(a.date, f"cell-{i:03d}", cfg, sched, rng, smoke=a.smoke)
            print(f"cell {i:03d} sched={sched} probe={adv} hits={hits}", flush=True)
        ...
        for dim in DIMENSIONS:
            ...
            hits, adv = run_cell(a.date, f"min-{dim}", c2, sched, rng)
            if not hits: keep.append(dim)     # resetting it removed the hit
        print("minimal dimensions (reset removes the hit):", keep)
        for rep in range(3):
            c2 = {k: (DEFAULTS[k] if k in keep else cfg[k]) for k in cfg}
            hits, adv = run_cell(a.date, f"min-confirm-{rep}", c2, sched, rng)
